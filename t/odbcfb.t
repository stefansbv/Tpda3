...
my $pass;
my $tempdir;
my $have_fb_driver = 1; # assume DBD::ODBC is installed and so is Firebird
my $live_testing   = 0;
...
END {
    ...
    my $dsn = qq{dbi:ODBC:dbname=$db_path;host=localhost;port=3050};
...
my $uri = "db:odbc://$user:$pass\@localhost/$db_path";
DBIEngineTest->run(
    class           => $CLASS,
    ...
    skip_unless => sub {
        my $self = shift;
        #die $err if $err;
        return 0 unless $have_fb_driver;    # skip if no DBD::ODBC
        $live_testing = 1;
    },
